...
export const getUserByHandle = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findOne({
      ...
      .select("-password -email"); // No enviar datos sensibles

    if (!user) {
      return res.status(404).json({ error: "Usuario no encontrado" });
    }

    // Si hay un usuario autenticado, verificar si lo sigue
    ...
    res.json({
      ...user.toObject(),
      isFollowing, // Incluir si el usuario actual lo sigue
    });
  } catch (error: any) {
...
export * from "./index"; // si tienes este archivo, o cualquier otro
